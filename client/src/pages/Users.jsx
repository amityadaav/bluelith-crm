...
//       const usersWithSafeData = (res.data || []).map(user => ({
//         ...user,
...
//       }));
//       setUsers(usersWithSafeData);
//       setFilteredUsers(usersWithSafeData);
//     } catch (error) {
...
//       alert("Error deleting user. They might have associated records.");
//     }
//   };
...
//       alert("Error deleting some users. They might have associated records.");
//     }
//   };
...
//     if (bulkSelect.length === filteredUsers.length && filteredUsers.length > 0) {
//       setBulkSelect([]);
//     } else {
...
//                         style={{ 
//                           backgroundColor: `${statusColors[user.status] || "#94a3b8"}20`,
...
//                     className="status-badge"
//                     style={{ 
//                       backgroundColor: `${statusColors[selectedUser.status] || "#94a3b8"}20`,
//                       color: statusColors[selectedUser.status] || "#94a3b8"
//                     }}
//                   >
...
//                     <div>
//                       <label>Role</label>
//                       <p>{selectedUser.role || "employee"}</p>
//                     </div>
//                   </div>
...
//     setLoading(true);
//     try {
//       const { confirmPassword, address, city, state, country, zipCode, ...rest } = formData;

//       // Nest address fields into embedded object to match MongoDB schema
//       const submitData = {
//         ...rest,
//         address: {
//           street: address || "",
...
//                 {(selectedUser.address?.street || selectedUser.address) && (
//                   <div className="details-section">
//                     <h4>Address</h4>
//                     <p>
//                       {selectedUser.address?.street || selectedUser.address}<br />
//                       {(selectedUser.address?.city || selectedUser.city) && `${selectedUser.address?.city || selectedUser.city}, `}
//                       {(selectedUser.address?.state || selectedUser.state) && `${selectedUser.address?.state || selectedUser.state} `}
//                       {selectedUser.address?.zipCode || selectedUser.zipCode}<br />
//                       {selectedUser.address?.country || selectedUser.country}
//                     </p>
//                   </div>
//                 )}
...
function Users() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      console.log("Fetched users:", raw.length, raw); // debug

      const usersWithSafeData = raw.map(user => ({
        ...user,
        permissions: Array.isArray(user.permissions) ? user.permissions : [],
        status: user.status || "active",
        role: user.role || "employee",
