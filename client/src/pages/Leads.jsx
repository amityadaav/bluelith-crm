...
//   // ─── FIX: safely extract the array no matter what shape the API returns ───
//   const extractArray = (data) => {
//     if (Array.isArray(data)) return data;
//     if (data && Array.isArray(data.leads)) return data.leads;
//     if (data && Array.isArray(data.data)) return data.data;
//     if (data && Array.isArray(data.results)) return data.results;
//     console.warn('fetchLeads: unexpected response shape', data);
//     return [];
//   };
...
//       conversionRate: safeLeads.length
//         ? ((safeLeads.filter(l => l.status === 'won').length / safeLeads.length) * 100).toFixed(1)
//         : 0
//     };
//     setStats(newStats);
//   };
...
import {
  ...
  Eye, Star, MessageCircle, Clock, CheckCircle, AlertCircle,
  TrendingUp, Users, DollarSign, Target, Send, Copy, UserPlus, Globe
} from 'lucide-react';
...
function Leads() {
  ...
  const [stats, setStats] = useState({
    total: 0, new: 0, contacted: 0, qualified: 0, lost: 0, conversionRate: 0
  });

  // ✅ FIX: Only fields that exist in the Lead schema
  const emptyForm = {
    name: '',
    email: '',
    phone: '',
  ...
  // ✅ FIX: Matches backend enum exactly — ["website","referral","linkedin","cold-call","email","other"]
  const sourceOptions = [
  ...
  useEffect(() => { filterAndSortLeads(); }, [leads, searchTerm, statusFilter, sortBy]);
  useEffect(() => { calculateStats(); }, [leads]);
  ...
  const fetchLeads = async () => {
    ...
    try {
      ...
      console.error('Error fetching leads:', error);
      setErrorMsg('Failed to load leads. The server may be waking up — please try again in a moment.');
      setLeads([]);
      setFilteredLeads([]);
    } finally {
      setLoading(false);
  ...
  const filterAndSortLeads = () => {
    ...
    switch (sortBy) {
      ...
      case 'value-high': filtered.sort((a, b) => (b.estimatedValue || 0) - (a.estimatedValue || 0)); break;
      case 'value-low':  filtered.sort((a, b) => (a.estimatedValue || 0) - (b.estimatedValue || 0)); break;
  ...
  const calculateStats = () => {
    ...
    setStats({
      total: safe.length,
      new: safe.filter(l => l.status === 'new').length,
      contacted: safe.filter(l => l.status === 'contacted').length,
      qualified: safe.filter(l => l.status === 'qualified').length,
      lost: safe.filter(l => l.status === 'lost').length,
      conversionRate: safe.length
        ? ((safe.filter(l => l.status === 'won').length / safe.length) * 100).toFixed(1)
        : 0,
    });
  };
  ...
  const handleOpen = (lead = null) => {
    ...
    if (lead) {
      ...
      setFormData({
        ...
        phone:          lead.phone          || '',
        company:        lead.company        || '',
        // ✅ FIX: validate source against backend enum
        source:         sourceOptions.find(s => s.value === lead.source) ? lead.source : 'other',
        status:         lead.status         || 'new',
        priority:       lead.priority       || 'medium',
        estimatedValue: lead.estimatedValue || '',
        notes:          lead.notes          || '',
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ FIX: Only send fields that exist in the Lead schema
    const payload = {
      name:           formData.name,
    ...
    try {
      ...
      console.error('Error saving lead:', error);
      setErrorMsg(error.response?.data?.message || 'Error saving lead. Please check all fields.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const bulkDelete = async () => {
    try {
      await Promise.all(bulkSelect.map(id => api.delete(`/leads/${id}`)));
      setBulkSelect([]); setShowBulkActions(false); fetchLeads();
    } catch (error) { console.error('Error bulk deleting:', error); }
  };

  const bulkStatusUpdate = async (status) => {
    try {
      await Promise.all(bulkSelect.map(id => api.put(`/leads/${id}`, { status })));
      setBulkSelect([]); setShowBulkActions(false); fetchLeads();
  ...
  const addActivity = async (leadId) => {
    ...
    try {
      await api.post(`/leads/${leadId}/activity`, {
        text: activityText, type: 'note', createdAt: new Date().toISOString()
      });
      setActivityText(''); setShowActivityLog(false); fetchLeads();
    } catch (error) { console.error('Error adding activity:', error); }
  };

  const toggleSelectAll = () => {
    setBulkSelect(bulkSelect.length === filteredLeads.length ? [] : filteredLeads.map(l => l._id));
  };
  const toggleSelect = (id) => {
  ...
  return (
    <div className="leads-container">
      ...
      <div className="leads-header">
        <div className="header-left">
          <h1><Target size={28} /> Lead Management</h1>
          <p className="header-subtitle">Track and manage your sales pipeline</p>
        </div>
        <div className="header-actions">
          <button className="btn-primary" onClick={() => handleOpen()}><Plus size={18} /> Add Lead</button>
          <button className="btn-secondary" onClick={() => handleExport('csv')}><Download size={18} /> Export</button>
      ...
      {errorMsg && (
        <div style={{
          ...
          <button onClick={() => setErrorMsg('')} style={{background:"none",border:"none",cursor:"pointer"}}><X size={16} /></button>
        </div>
      )}

      {/* Stats */}
      <div className="stats-grid">
        <div className="stat-card"><div className="stat-icon total"><Users size={24} /></div><div className="stat-content"><h3>Total Leads</h3><div className="stat-value">{stats.total}</div></div></div>
        <div className="stat-card"><div className="stat-icon new"><UserPlus size={24} /></div><div className="stat-content"><h3>New</h3><div className="stat-value">{stats.new}</div></div></div>
        <div className="stat-card"><div className="stat-icon contacted"><Mail size={24} /></div><div className="stat-content"><h3>Contacted</h3><div className="stat-value">{stats.contacted}</div></div></div>
        <div className="stat-card"><div className="stat-icon qualified"><CheckCircle size={24} /></div><div className="stat-content"><h3>Qualified</h3><div className="stat-value">{stats.qualified}</div></div></div>
        <div className="stat-card"><div className="stat-icon conversion"><TrendingUp size={24} /></div><div className="stat-content"><h3>Conversion Rate</h3><div className="stat-value">{stats.conversionRate}%</div></div></div>
      </div>
        ...
        <div className="filter-group">
          ...
          {bulkSelect.length > 0 && (
            <div className="bulk-actions">
              ...
              {showBulkActions && (
                <div className="bulk-dropdown">
                  <button onClick={() => bulkStatusUpdate('new')}><UserPlus size={16} /> Mark New</button>
                  <button onClick={() => bulkStatusUpdate('contacted')}><Mail size={16} /> Mark Contacted</button>
                  <button onClick={() => bulkStatusUpdate('qualified')}><CheckCircle size={16} /> Mark Qualified</button>
                  <button onClick={() => bulkStatusUpdate('won')}><TrendingUp size={16} /> Mark Won</button>
                  <button onClick={() => bulkStatusUpdate('lost')}><AlertCircle size={16} /> Mark Lost</button>
      ...
      <div className="table-container">
        <table className="leads-table">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan="10" className="loading-cell"><div className="spinner"></div>Loading leads...</td></tr>
            ) : filteredLeads.length === 0 ? (
              <tr>
                <td colSpan="10" className="no-data">
                  <Target size={48} /><p>No leads found</p>
                  <button onClick={() => handleOpen()} className="btn-primary">Add your first lead</button>
                </td>
              </tr>
            ) : (
              filteredLeads.map((lead) => {
                ...
                return (
                  <tr key={lead._id} className={bulkSelect.includes(lead._id) ? 'selected' : ''}>
                    <td><input type="checkbox" checked={bulkSelect.includes(lead._id)} onChange={() => toggleSelect(lead._id)} /></td>
                    <td>
                      <div className="lead-info">
                    ...
                    <td>
                      <div className="contact-info">
                        <div className="contact-item"><Mail size={14} /><span>{lead.email}</span></div>
                        {lead.phone && <div className="contact-item"><Phone size={14} /><span>{lead.phone}</span></div>}
                      </div>
                    </td>
                    <td><div className="company-info"><Building size={14} /><span>{lead.company || 'N/A'}</span></div></td>
                    <td>
                    ...
                    <td><span className="value-amount">₹{(lead.estimatedValue || 0).toLocaleString()}</span></td>
                    <td><span className="source-badge">{lead.source}</span></td>
                    <td>
                      <div className="time-info">
                        <Calendar size={14} />
                        <span>{lead.followUpDate ? new Date(lead.followUpDate).toLocaleDateString() : '—'}</span>
                      </div>
                    </td>
                    <td>
                      <div className="action-buttons">
                        <button onClick={() => { setSelectedLead(lead); setShowDetails(true); }} className="action-btn view" title="View"><Eye size={16} /></button>
                        <button onClick={() => handleOpen(lead)} className="action-btn edit" title="Edit"><Edit size={16} /></button>
                        <button onClick={() => handleCopyLead(lead)} className="action-btn copy" title="Copy"><Copy size={16} /></button>
                        <button onClick={() => { setSelectedLead(lead); setShowActivityLog(true); }} className="action-btn activity" title="Activity"><MessageCircle size={16} /></button>
                        <button onClick={() => setShowDeleteConfirm(lead._id)} className="action-btn delete" title="Delete"><Trash2 size={16} /></button>
                      </div>
                      {showDeleteConfirm === lead._id && (
                        <div className="delete-confirm">
                          <p>Delete this lead?</p>
            ...
            <form onSubmit={handleSubmit} className="lead-form">
              <div className="form-section">
                ...
                <div className="form-grid">
                  ...
                  <div className="form-group">
                    <label>Email *</label>
                    <input type="email" name="email" value={formData.email} onChange={handleChange} required placeholder="lead@email.com" />
                  </div>
                  <div className="form-group">
                    <label>Phone</label>
              ...
              <div className="form-section">
                ...
                <div className="form-grid">
                  ...
                  {/* ✅ FIX: Only valid source values from backend enum */}
                  <div className="form-group">
                    <label>Source</label>
                  ...
                  <div className="form-group">
                    <label>Estimated Value (₹)</label>
                    <input type="number" name="estimatedValue" value={formData.estimatedValue} onChange={handleChange} placeholder="0" min="0" />
                  </div>
                  <div className="form-group">
              ...
              <div className="form-section">
                <h3>Notes</h3>
                <div className="form-group">
                  <textarea name="notes" value={formData.notes} onChange={handleChange} rows="4" placeholder="Additional notes about the lead..." />
                </div>
              </div>
      ...
      {showDetails && selectedLead && (
        <div className="modal-overlay" onClick={() => setShowDetails(false)}>
          <div className="modal-content details-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="details-container">
              <div className="details-header">
                <div className="lead-avatar large">{selectedLead.name?.charAt(0).toUpperCase()}</div>
                <div className="lead-title">
                  <h3>{selectedLead.name}</h3>
                ...
                <div className="lead-badges">
                  <span className="status-badge" style={{
                    backgroundColor:`${getStatusBadge(selectedLead.status).color}20`,
                    color:getStatusBadge(selectedLead.status).color
                  }}>{getStatusBadge(selectedLead.status).label}</span>
                  <span className="priority-badge" style={{
                    backgroundColor:`${priorityColors[selectedLead.priority]}20`,
                    color:priorityColors[selectedLead.priority]
                  }}>{selectedLead.priority} priority</span>
                </div>
              </div>
              <div className="details-grid">
                <div className="details-section">
                  <h4>Contact</h4>
                  <div className="detail-item"><Mail size={16} /><div><label>Email</label><p>{selectedLead.email}</p></div></div>
                  {selectedLead.phone && <div className="detail-item"><Phone size={16} /><div><label>Phone</label><p>{selectedLead.phone}</p></div></div>}
                </div>
                <div className="details-section">
                  <h4>Lead Info</h4>
                  <div className="detail-item"><Target size={16} /><div><label>Source</label><p>{selectedLead.source}</p></div></div>
                  <div className="detail-item"><DollarSign size={16} /><div><label>Estimated Value</label><p>₹{(selectedLead.estimatedValue || 0).toLocaleString()}</p></div></div>
                </div>
                <div className="details-section">
                  <h4>Timeline</h4>
                  <div className="detail-item"><Calendar size={16} /><div><label>Created</label><p>{new Date(selectedLead.createdAt).toLocaleDateString()}</p></div></div>
                  {selectedLead.followUpDate && <div className="detail-item"><Clock size={16} /><div><label>Follow-up</label><p>{new Date(selectedLead.followUpDate).toLocaleDateString()}</p></div></div>}
                </div>
                {selectedLead.notes && <div className="details-section full-width"><h4>Notes</h4><p className="notes-text">{selectedLead.notes}</p></div>}
              </div>
              <div className="details-actions">
                <button className="btn-secondary" onClick={() => { setShowDetails(false); handleOpen(selectedLead); }}><Edit size={16} /> Edit Lead</button>
                <button className="btn-primary" onClick={() => { window.location.href = `mailto:${selectedLead.email}`; }}><Mail size={16} /> Send Email</button>
                <button className="btn-secondary" onClick={() => { setShowDetails(false); setShowActivityLog(true); }}><MessageCircle size={16} /> Add Activity</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showActivityLog && selectedLead && (
        <div className="modal-overlay" onClick={() => setShowActivityLog(false)}>
          <div className="modal-content activity-modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Add Activity — {selectedLead.name}</h2>
              <button onClick={() => setShowActivityLog(false)} className="close-btn"><X size={20} /></button>
            </div>
            <div className="activity-container">
              <div className="activities-list">
                <h3>Recent Activities</h3>
                {!selectedLead.activities || selectedLead.activities.length === 0 ? (
                  <div className="no-activities"><MessageCircle size={48} /><p>No activities yet</p></div>
                ) : (
                  selectedLead.activities.map((activity, index) => (
                    <div key={index} className="activity-item">
                      <div className="activity-content">
              ...
              <div className="add-activity">
                ...
                <textarea value={activityText} onChange={(e) => setActivityText(e.target.value)}
                  placeholder="Write a note, log a call, record a meeting..." rows="4" />
                <button onClick={() => addActivity(selectedLead._id)} className="btn-primary" disabled={!activityText.trim()}>
                  <Send size={16} /> Add Activity
                </button>
              </div>
            </div>
