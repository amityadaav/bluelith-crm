...
const Projects = () => {
  ...
  const [showBulkActions,   setShowBulkActions]   = useState(false);
  const [stats,             setStats]             = useState({
    total: 0, completed: 0, inProgress: 0, pending: 0, totalBudget: 0, totalPaid: 0
  });

  const [form, setForm] = useState({
    name: "", client: "", clientEmail: "", clientPhone: "",
    assignedDeveloper: "", assignedDeveloperEmail: "",
    totalBudget: "", paid: "", deadline: "", description: "",
  ...
  useEffect(() => { filterAndSortProjects(); }, [projects, searchTerm, statusFilter, sortBy]);
  useEffect(() => { calculateStats(); }, [projects]);
  ...
  const extractArray = (data) => {
    ...
    console.warn("fetchProjects: unexpected shape", data);
    return [];
  };
  ...
  const filterAndSortProjects = () => {
    ...
    switch (sortBy) {
      ...
      case "deadline":    filtered.sort((a, b) => new Date(a.deadline)  - new Date(b.deadline));  break;
      case "budget-high": filtered.sort((a, b) => b.totalBudget - a.totalBudget); break;
      case "budget-low":  filtered.sort((a, b) => a.totalBudget - b.totalBudget); break;
      default: break;
    }
  ...
  const calculateStats = () => {
    ...
    setStats({
      total:       safe.length,
      completed:   safe.filter(p => p.status === "Completed").length,
      inProgress:  safe.filter(p => p.status === "In Progress").length,
      pending:     safe.filter(p => p.status === "Pending").length,
      totalBudget: safe.reduce((s, p) => s + (p.totalBudget || 0), 0),
      totalPaid:   safe.reduce((s, p) => s + (p.paid        || 0), 0),
    });
  };
  ...
  const bulkStatusUpdate = async (status) => {
    try {
      await Promise.all(bulkSelect.map(id => API.put(`/projects/${id}/status`, { status })));
      setBulkSelect([]);
      setShowBulkActions(false);
      fetchProjects();
    } catch (error) {
  ...
  const addComment = async (id) => {
    if (!commentText.trim()) return;
    try {
      await API.post(`/projects/${id}/comment`, { text: commentText, createdAt: new Date().toISOString() });
      setCommentText("");
      fetchProjects();
    } catch (error) {
      console.error("Error adding comment:", error);
  ...
  const handleExport = (format) => {
    ...
    const data = filteredProjects.map(p => ({
      ...
      Budget:    p.totalBudget,
      Paid:      p.paid,
      Remaining: (p.totalBudget || 0) - (p.paid || 0),
  ...
  const toggleSelectAll = () => {
    setBulkSelect(bulkSelect.length === filteredProjects.length ? [] : filteredProjects.map(p => p._id));
  };
  ...
  return (
    <div className="projects-container">
      ...
      <div className="projects-header">
        <div className="header-left">
          <h1><FileText size={28} /> Project Management</h1>
          <p className="header-subtitle">Manage and track all your projects in one place</p>
        </div>
        <div className="header-actions">
          ...
          <button className="btn-refresh" onClick={fetchProjects} disabled={loading}>
            <RefreshCw size={18} className={loading ? "spin" : ""} />
          </button>
        </div>
      </div>
      ...
      <div className="stats-grid">
        <div className="stat-card">
          ...
          <div className="stat-content"><h3>Total Projects</h3><div className="stat-value">{stats.total}</div></div>
        </div>
        <div className="stat-card">
          <div className="stat-icon completed"><CheckCircle size={24} /></div>
          <div className="stat-content"><h3>Completed</h3><div className="stat-value">{stats.completed}</div></div>
        </div>
        <div className="stat-card">
          <div className="stat-icon progress"><Clock size={24} /></div>
          <div className="stat-content"><h3>In Progress</h3><div className="stat-value">{stats.inProgress}</div></div>
        </div>
        <div className="stat-card">
          <div className="stat-icon pending"><AlertCircle size={24} /></div>
          <div className="stat-content"><h3>Pending</h3><div className="stat-value">{stats.pending}</div></div>
        </div>
        <div className="stat-card">
          <div className="stat-icon budget"><DollarSign size={24} /></div>
          <div className="stat-content"><h3>Total Budget</h3><div className="stat-value">₹{stats.totalBudget.toLocaleString()}</div></div>
        </div>
        <div className="stat-card">
          <div className="stat-icon paid"><DollarSign size={24} /></div>
          <div className="stat-content"><h3>Total Paid</h3><div className="stat-value">₹{stats.totalPaid.toLocaleString()}</div></div>
        </div>
      </div>
        ...
        <div className="filter-group">
          ...
          {bulkSelect.length > 0 && (
            <div className="bulk-actions">
              ...
              {showBulkActions && (
                <div className="bulk-dropdown">
                  <button onClick={() => bulkStatusUpdate("Completed")}><CheckCircle size={16} /> Mark Completed</button>
                  <button onClick={() => bulkStatusUpdate("In Progress")}><Clock size={16} /> Mark In Progress</button>
                  <button onClick={() => bulkStatusUpdate("Pending")}><AlertCircle size={16} /> Mark Pending</button>
            ...
            <form onSubmit={handleSubmit} className="project-form">
              <div className="form-section">
                ...
                <div className="form-grid">
                  <div className="form-group">
                    <label>Project Name *</label>
                    <input name="name" value={form.name} onChange={handleChange} required placeholder="Enter project name" />
                  </div>
                  <div className="form-group">
                    <label>Category</label>
                    <input name="category" value={form.category} onChange={handleChange} placeholder="e.g., Web Development, Design" />
                  </div>
                  <div className="form-group full-width">
                    <label>Description</label>
              ...
              <div className="form-section">
                ...
                <div className="form-grid">
                  <div className="form-group">
                    <label>Client Name *</label>
                    <input name="client" value={form.client} onChange={handleChange} required placeholder="Enter client name" />
                  </div>
                  <div className="form-group">
                    <label>Client Email</label>
                    <input type="email" name="clientEmail" value={form.clientEmail} onChange={handleChange} placeholder="client@email.com" />
                  </div>
                  <div className="form-group">
                    <label>Client Phone</label>
                    <input name="clientPhone" value={form.clientPhone} onChange={handleChange} placeholder="Phone number" />
                  </div>
                </div>
              </div>
              ...
              <div className="form-section">
                ...
                <div className="form-grid">
                  <div className="form-group">
                    <label>Assigned Developer *</label>
                    <input name="assignedDeveloper" value={form.assignedDeveloper} onChange={handleChange} required placeholder="Developer name" />
                  </div>
                  <div className="form-group">
                    <label>Developer Email</label>
                    <input type="email" name="assignedDeveloperEmail" value={form.assignedDeveloperEmail} onChange={handleChange} placeholder="developer@email.com" />
                  </div>
                </div>
              </div>
              ...
              <div className="form-section">
                ...
                <div className="form-grid">
                  <div className="form-group">
                    <label>Total Budget (₹) *</label>
                    <input type="number" name="totalBudget" value={form.totalBudget} onChange={handleChange} required placeholder="0" />
                  </div>
                  <div className="form-group">
                    <label>Paid Amount (₹)</label>
                    <input type="number" name="paid" value={form.paid} onChange={handleChange} placeholder="0" />
                  </div>
                  <div className="form-group">
                    <label>Deadline *</label>
              ...
              <div className="form-section">
                ...
                <div className="tags-container">
                  {form.tags.map((tag, i) => (
                    <span key={i} className="tag">{tag}<X size={14} onClick={() => handleRemoveTag(tag)} /></span>
                  ))}
                </div>
              </div>
      ...
      <div className="table-container">
        <table className="projects-table">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan="11" className="loading-cell"><div className="spinner"></div>Loading projects...</td></tr>
            ) : filteredProjects.length === 0 ? (
              <tr>
                <td colSpan="11" className="no-data">
                  <FileText size={48} />
              ...
              filteredProjects.map((project) => (
                <tr key={project._id} className={bulkSelect.includes(project._id) ? "selected" : ""}>
                  ...
                  <td>
                    <div className="client-info">
                      <span className="client-name">{project.client}</span>
                      {project.clientEmail && <span className="client-email">{project.clientEmail}</span>}
                    </div>
                  </td>
                  ...
                  <td className="budget-col">
                    <span className="total-budget">₹{(project.totalBudget || 0).toLocaleString()}</span>
                  </td>

                  <td>
                    <div className="payment-info">
                      <span className="paid-amount">₹{(project.paid || 0).toLocaleString()}</span>
                      <span className="remaining-amount">
                        Remaining: ₹{((project.totalBudget || 0) - (project.paid || 0)).toLocaleString()}
                      </span>
                      <div className="payment-progress">
                        <div
                          className="progress-bar"
                          style={{ width: `${project.totalBudget ? ((project.paid / project.totalBudget) * 100) : 0}%` }}
                        />
                      </div>
                    </div>
                  </td>
                  ...
                  <td>
                    <div className="deadline-info">
                      ...
                      <span className={`deadline-status ${getRemainingDays(project.deadline) === "Overdue" ? "overdue" : ""}`}>
                        {getRemainingDays(project.deadline)}
                      </span>
                    </div>
                  </td>
                  ...
                  <td>
                    <button
                      onClick={() => { setSelectedProject(project); setShowComments(true); }}
                      className="comments-toggle"
                    >
                      <MessageCircle size={16} />
                      <span>{project.comments?.length || 0}</span>
                  ...
                  <td>
                    <div className="action-buttons">
                      <button onClick={() => handleEdit(project)} className="action-btn edit" title="Edit">
                        <Edit size={16} />
                      </button>
                      <button onClick={() => handleCopyProject(project)} className="action-btn copy" title="Copy">
                        <Copy size={16} />
                      </button>
                      <button onClick={() => setShowDeleteConfirm(project._id)} className="action-btn delete" title="Delete">
                        <Trash2 size={16} />
                      </button>
                    </div>
      ...
      {showComments && selectedProject && (
        <div className="modal-overlay" onClick={() => setShowComments(false)}>
          <div className="modal-content comments-modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Comments — {selectedProject.name}</h2>
              <button onClick={() => setShowComments(false)} className="close-btn"><X size={20} /></button>
            </div>
            ...
            <div className="comments-container">
                ...
                {!selectedProject.comments || selectedProject.comments.length === 0 ? (
                  ...
                  selectedProject.comments.map((comment, index) => (
                    <div key={index} className="comment-item">
                      ...
                      <p className="comment-text">{comment.text}</p>
                      <button onClick={() => deleteComment(selectedProject._id, comment._id)} className="delete-comment">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  ))
              ...
              <div className="add-comment">
                ...
                />
                <button
                  onClick={() => addComment(selectedProject._id)}
                  className="btn-primary"
                  disabled={!commentText.trim()}
                >
                  Post Comment
